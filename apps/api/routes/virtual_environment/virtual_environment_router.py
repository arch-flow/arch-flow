...
from apps.modules.virtual_environment.application.dto.output_virtual_environment_dto import \
    VirtualEnvironmentOutputDTO
...
@router.post("/", status_code=status.HTTP_201_CREATED, response_model=VirtualEnvironmentOutputDTO)
def create_virtual_environment(dto: CreateVirtualEnvironmentDTO, session: Session = Depends(get_session)):
    ...
    return VirtualEnvironmentOutputDTO.from_entity(virtual_created)


@router.get("/", response_model=list[VirtualEnvironmentOutputDTO])
def list_virtual_environments(offset: int = Query(0), limit: int = Query(10), session: Session = Depends(get_session)):
    ...
    return [VirtualEnvironmentOutputDTO.from_entity(environment) for environment in virtual_environments]


@router.get("/{environment_id}", response_model=VirtualEnvironmentOutputDTO)
def get_virtual_environment(environment_id: UUID, session: Session = Depends(get_session)):
    ...
    return VirtualEnvironmentOutputDTO.from_entity(virtual_environment)


@router.put("/{environment_id}", response_model=VirtualEnvironmentOutputDTO)
